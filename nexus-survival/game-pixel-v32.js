...
drawEnemy=function(e){if(!ready||!e||enemyAtlasFailed)return oldEnemy(e);const key=keyForEnemy(e);if(enemyAtlasReady&&!enemyRegionOK.has(key))return oldEnemy(e);const t=g?.t||0,baseR=e.r||18,swift=e.variantRole==='swift',brute=e.variantRole==='brute',bob=Math.sin((e.anim||t*6)+(Number(e.id)||0)*.31)*(e.boss?1.4:swift?2.0:1.15);let h=e.boss?(stageNo()===50?116:(e.bossTier==='mid'?102:96)):(e.elite?53:brute?48:swift?41:44);h*=clamp(baseR/18,.82,e.boss?1.25:1.38);const face=(e.vx??0)>=0,rot=swift?Math.sin(t*7+(Number(e.id)||0))*.035:0,glow=e.boss?'#ff744f':e.elite?(e.gradeColor||'#e0bd70'):null;if(!sprite(key,e.x,e.y+bob,h,{flip:face,rot,glow,glowBlur:e.boss?10:5,enemy:true}))return oldEnemy(e);shadow(e.x,e.y,e.boss?Math.min(100,h*.72):Math.max(32,h*.72));gradeRing(e);enemyOverlay(e)};
