...
function sprite(key,x,y,targetH,opt={}){const a=MAP[key];if(!a||!ready)return false;const[sx,sy,sw,sh]=a,scale=targetH/sh,dw=Math.round(sw*scale),dh=Math.round(targetH),bottom=opt.bottom??Math.round(y+20),dx=-dw/2,dy=-dh;ctx.save();ctx.imageSmoothingEnabled=false;ctx.translate(Math.round(x),bottom);if(opt.rot)ctx.rotate(opt.rot);ctx.scale(opt.flip===false?1:-1,1);ctx.globalAlpha=opt.alpha??1;if(opt.glow){ctx.shadowColor=opt.glow;ctx.shadowBlur=opt.glowBlur||8}ctx.drawImage(img,sx,sy,sw,sh,dx,dy,dw,dh);ctx.restore();return true}
...
drawEnemy=function(e){if(!ready||!e)return oldEnemy(e);const key=keyForEnemy(e),t=g?.t||0,baseR=e.r||18,swift=e.variantRole==='swift',brute=e.variantRole==='brute',bob=Math.sin((e.anim||t*6)+(Number(e.id)||0)*.31)*(e.boss?1.4:swift?2.0:1.15);let h=e.boss?(stageNo()===50?116:(e.bossTier==='mid'?102:96)):(e.elite?53:brute?48:swift?41:44);h*=clamp(baseR/18,.82,e.boss?1.25:1.38);shadow(e.x,e.y,e.boss?Math.min(100,h*.72):Math.max(32,h*.72));gradeRing(e);const face=(e.vx??0)>=0,rot=swift?Math.sin(t*7+(Number(e.id)||0))*.035:0,glow=e.boss?'#ff744f':e.elite?(e.gradeColor||'#e0bd70'):null;sprite(key,e.x,e.y+bob,h,{flip:face,rot,glow,glowBlur:e.boss?10:5});enemyOverlay(e)};
