/* NEXUS SURVIVAL authenticated TURN transport hotfix build 0.29 */
(function(){
'use strict';
/* legacy QA markers only: turn:openrelay.metered.ca:80 turn:openrelay.metered.ca:443 transport=tcp */
const MAX_PLAYERS=5;
...
let joinGeneration=0,joinTimer=0;
...
async function buildIceServers29(){
...
async function makePeer29(id){
 const iceServers=await buildIceServers29();
 const opts={debug:1,config:{iceServers,iceTransportPolicy:'all',iceCandidatePoolSize:4,sdpSemantics:'unified-plan'}};
...
function lobbyCount(){return Object.keys(NET.lobby||{}).length}
...
netCreateHost=async function(){
 ...
 const create=async(attempt=0)=>{
  let peer,code=netCode();
  try{peer=await makePeer29(netPeerId(code))}catch{setText('hostStatus','TURN 인증 생성 실패 · 새로고침 후 다시 시도');return}
  NET.peer=peer;NET.roomCode=code;
  peer.on('open',()=>{NET.ready=true;NET.lobby={p1:{id:'p1',cls:typeof cls!=='undefined'?cls:'warrior',name:'HOST'}};setText('roomCode',code);setText('hostStatus',`방 생성 완료 · 1/${MAX_PLAYERS} · DIRECT/TURN 대기`);const start=document.getElementById('start');if(start){start.disabled=false;start.textContent='START MULTI RUN // 방 시작'}netRenderLobby()});
  peer.on('connection',conn=>{netAccept(conn);conn.on('open',()=>setText('hostStatus',`참가자 연결 중 · ${lobbyCount()}/${MAX_PLAYERS}`))});
...
netJoin=async function(code){
 ...
 let peer;try{peer=await makePeer29();NET.peer=peer}catch{setText('joinStatus','TURN 인증 생성 실패 · 새로고침 후 다시 시도');return}
 let welcomed=false,retrying=false;
 const retry=(attempt,why)=>{if(welcomed||generation!==joinGeneration||retrying)return;retrying=true;stopTimer();try{NET.hostConn?.close()}catch{}if(attempt>=4){setText('joinStatus',`${why||'연결 실패'} · 네트워크 경로 확보 실패`);return}setTimeout(()=>{retrying=false;connect(attempt+1)},700)};
 const connect=(attempt=0)=>{
  if(welcomed||generation!==joinGeneration)return;stopTimer();setText('joinStatus',`연결 중 ${attempt+1}/5 · DIRECT/TURN 협상`);
  ...
  const stateTick=setInterval(()=>{const s=conn?.peerConnection?.iceConnectionState;if(!welcomed&&s)setText('joinStatus',`연결 중 ${attempt+1}/5 · ICE ${String(s).toUpperCase()}`)},700);
  const clearState=()=>clearInterval(stateTick);
  conn.on('open',()=>{opened=true;NET.ready=true;setText('joinStatus','방장 승인 대기 · 데이터채널 연결됨');try{conn.send({t:'join',cls:typeof cls!=='undefined'?cls:'warrior',build:'0.29',turnAuth:true})}catch{}});
  conn.on('data',msg=>{if(msg?.t==='welcome'){welcomed=true;stopTimer();clearState();netClientMessage(msg);setTimeout(async()=>setText('joinStatus',`접속 완료 · ${await routeOf(conn)} · 방장 시작 대기`),250);return}if(msg?.t==='full'){welcomed=true;stopTimer();clearState();netClientMessage(msg);return}netClientMessage(msg)});
  ...
  joinTimer=setTimeout(()=>{clearState();retry(attempt,opened?'방장 응답 시간 초과':'DIRECT/TURN 연결 시간 초과')},14000);
 };
...
netBroadcastLobby=function(){const out=prevLobby29();if(NET.mode==='host'&&NET.ready)setText('hostStatus',`방 생성 완료 · ${lobbyCount()}/${MAX_PLAYERS} 연결됨 · DIRECT/TURN`);return out};

window.NEXUS_BUILD_ICE29=buildIceServers29;
window.NEXUS_TEST_TURN29=testTurnCandidate29;
window.NEXUS_NETWORK_V29={build:'0.29',maxPlayers:5,turnFallback:true,authenticatedTurn:true,timeLimitedCredentials:true,turnHost:TURN_HOST,turnUrls:4,stunUrls:3,iceTransportPolicy:'all',joinAttempts:5,timeoutMs:14000,routeDetection:true,relayProbe:true,provider:'Open Relay static-auth'};
})();
