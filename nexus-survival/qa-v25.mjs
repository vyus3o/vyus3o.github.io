...
try{
 ...
 const cross=await p.evaluate(()=>eval(`(()=>{const q=localPlayer();state='play';const result={};const cases=[['warrior',{W01:1,W06:1},'warrior-blood-cyclone'],['mage',{M01:1,M02:1},'mage-plasma'],['priest',{P03:1,P09:1},'priest-blessed-sanctuary'],['necromancer',{N03:1,N04:1},'necro-corpse-legion'],['rogue',{R02:1,R08:1},'rogue-venom-blood'],['gunslinger',{GS03:1,GS04:1},'gun-explosive-ricochet']];for(const [c,skills,id] of cases){q.cls=c;q.skills=skills;q.advSkills={};q.subclass=null;q.cd={};q._v25SynergyCd={[id]:-1};q.atkBuff=1;q.atkBuffUntil=0;g.e=[];g.zones=[];g.fx=[];g.q=[];spawn(false);const e=g.e.find(x=>!x.boss);e.x=q.x+90;e.y=q.y;const hp=e.hp,sh=q.shield||0,buff=q.atkBuff||1;runSkills(q,.016);result[c]={damage:e.hp<hp,fx:g.fx.length,z:g.zones.length,q:g.q.length,shield:(q.shield||0)>sh,summon:(q._v25SummonUntil||0)>g.t,buff:(q.atkBuff||1)>buff&&(q.atkBuffUntil||0)>g.t}}return result})()`));
 for(const [c,v] of Object.entries(cross))if(!(v.damage||v.fx||v.z||v.q||v.shield||v.summon||v.buff))fail('cross-class combo '+c,JSON.stringify(v));mark('warrior/mage/priest/necro/rogue/gunslinger combo effects PASS');
