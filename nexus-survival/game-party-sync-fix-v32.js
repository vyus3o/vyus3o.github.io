/* NEXUS SURVIVAL party level sync compatibility/UI hotfix 0.32.3 */
(function(){
'use strict';
...
function hostWait(){
 ...
 m.classList.remove('hidden');m.classList.add('partyPause28');box.innerHTML='<div style="grid-column:1/-1;padding:28px 16px;text-align:center;color:#c4d2c7;font:800 12px/1.7 monospace;letter-spacing:.5px">PARTY PAUSED<br>다른 플레이어의 레벨업 선택을 기다리는 중...</div>';if(nx)nx.disabled=true;
}
...
const oldRemote=netApplyRemoteChoice;netApplyRemoteChoice=function(pid,o){const out=oldRemote(pid,o);settle('remote-choice');return out};
...
requestLevel=function(p){
 ...
 for(const x of players){
  x.pendingLevel=true;
  if(x!==p&&Number.isFinite(p.l)){x.l=p.l;x.xp=p.xp;x.nxp=p.nxp}
  ...
  else{const c=[...NET.conns.values()].find(c=>c?.playerId===x.id&&c.open);if(c)try{c.send({t:'v32LevelOffer',round:r.id,playerId:x.id,choices:JSON.parse(JSON.stringify(r.choices[x.id])),build:'0.32.3'})}catch{}}
 }
...
gainPartyXp=function(v){
 if(!g||NET.mode==='solo')return oldGainPartyXp(v);if(NET.mode==='client')return;
 const ps=Object.values(g.players||{}).filter(x=>x&&x.alive!==false);if(!ps.length)return;
 if(ps.some(x=>x.pendingLevel)||window.NEXUS_PARTY_SYNC_V32?.hostRound)return;
 const leader=g.players?.p1||ps[0];leader.xp=(Number(leader.xp)||0)+(Number(v)||0);
 for(const x of ps){x.xp=leader.xp;x.l=leader.l;x.nxp=leader.nxp}
 if(leader.xp<leader.nxp)return;
 leader.xp-=leader.nxp;leader.l=(leader.l||1)+1;leader.nxp=Math.floor(35*Math.pow(leader.l,1.18));
 for(const x of ps){x.xp=leader.xp;x.l=leader.l;x.nxp=leader.nxp}
 requestLevel(leader);
};
const oldSel=selectChoice;selectChoice=function(o){const out=oldSel(o);settle('local-choice');return out};
...
window.NEXUS_PARTY_SYNC_FIX_V32={build:'0.32.3',immediateLastChoiceResume:true,hostWaitingUi:true,legacyRemoteChoiceCompatible:true,partyWideLevelChoices:true,sharedXp:true,singleCanonicalRound:true};
})();